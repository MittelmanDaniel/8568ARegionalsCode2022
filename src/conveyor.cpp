...
const int JAMMED_IF_VEL_FOR = 200; // Conveyor needs to be jammed for this long before it tries to unjam (ms)
const int CONVEYOR_REVERSE_STUCK = 700; // When the conveyor jams, conveyor will run backwards for this amount of time (ms)
const int CONVEYOR_WAS_STUCK   = 100; // After getting jammed, the code cannot get stuck again for this amount of time (ms)


pros::Motor conveyor(6, true);
...
set_conveyor(int input)  {
  ...
  if ((get_conveyor_velocity()==0) && input>100 && !was_stuck) {
    almost_stuck_timer+=ez::util::DELAY_TIME;
  ...
  if (is_stuck) {
    output = -input;
    stuck_timer += ez::util::DELAY_TIME;
  ...
  // After unjamming, the conveyor cannot get stuck again for CONVEYOR_WAS_STUCK
    ...
    if (was_stuck) {
      was_stuck_timer+= ez::util::DELAY_TIME;
...
///
// Driver Control
//  - when L2 is pressed, run the conveyor in
